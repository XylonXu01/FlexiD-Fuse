...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='./TripleSeries_output2')
    ...
    # Working directory
    test_folder=r"TripleSeries_crop"     
    ...
    for img_name in os.listdir(os.path.join(test_folder,"vi")):
        inf_img = image_read(os.path.join(test_folder,"3",img_name),mode='GRAY')[np.newaxis,np.newaxis, ...]/255.0 
        vis_img = image_read(os.path.join(test_folder,"vi",img_name), mode='GRAY')[np.newaxis,np.newaxis, ...]/255.0 
        # 如果img_3地址为空，将img_3设置为全0
        if os.path.exists(os.path.join(test_folder,"ir",img_name)):
            img_3 = image_read(os.path.join(test_folder,"ir",img_name), mode='GRAY')[np.newaxis,np.newaxis, ...]/255.0
        else:
        ...
        with torch.no_grad():
            sample = sample_fn(x_start=x_start, record=True, I = inf_img, V = vis_img, img_3 = img_3, save_root=out_path, img_index = os.path.splitext(img_name)[0], lamb=0.5,rho=0.001)
